  ...
  task(:permissions => :environment) do
    ...
    arr.each do |controller|
      ...
      if controller.permission
        ...
        write_permission(controller.permission, "all", 'all') #add permission to do CRUD for every model.
...
def eval_cancan_action(action)
  ...
  when "index"
    name = 'read'
    cancan_action = "read" #let the cancan action be the actual method name
    action_desc = :read
  when "new", "create"
    name = 'write'
    cancan_action = "write"
...
# Old method. This was changed to simplify to read, write, delete
# def eval_cancan_action(action)
#   case action.to_s
#   when "index"
#     name = 'list'
...
def write_permission(model, cancan_action, name)  
  if Permission.where(subject_class: model, action: cancan_action).count < 1
